...
def extract_words(vectors, word_list, relations):
    ...
    for i in range(m):
        vec0[i,:] = embedding.get_vec(rel[i][0], word_list, vectors)
        vec1[i,:] = embedding.get_vec(rel[i][1], word_list, vectors)
...
def main(args):
    vectors, word_list = embedding.load_glove_vectors(args.npyFILE)
